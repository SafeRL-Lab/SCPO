...
def approx_safety_delta(safe_ppo: "SafePPO2") -> Tuple[float, float]:
    delta = []
    delta_pessimistic = []
    with th.no_grad():
        for rollout_data in safe_ppo.rollout_buffer.get(safe_ppo.batch_size):
            log_prob = safe_ppo.policy.actions_log_prob(rollout_data.observations, rollout_data.actions)
            ratio = 1 - th.exp(rollout_data.old_log_prob - log_prob)
            safety_delta_pessimistic = (ratio * rollout_data.safety_value).mean()
            delta_pessimistic.append(safety_delta_pessimistic.item())

            r2 = th.exp(log_prob - rollout_data.old_log_prob)
            safety_delta_optimistic = (r2 * rollout_data.safety_return - rollout_data.safety_value).mean()
            delta.append(safety_delta_optimistic.item())

    return np.mean(delta_pessimistic), np.mean(delta)


class SafePPO2(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        safe_episode_mean = self.rollout_buffer.get_safe_episode_mean()
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                target = th.clamp(0.1*(rollout_data.safety_return * 2 - 1) + rollout_data.safety_value, 0, 1)
                safety_value_loss = F.smooth_l1_loss(target, safety_values)
                ...
                with th.no_grad():
                    clip_fraction = th.mean((th.abs(ratio) > clip_range).float()).item()
                    ...
                    approx_dl_divs.append((th.exp(log_prob) - 0.5).mean().item())
            if not continue_training:
                break

        last_safety_epoch_step_index = 0
        safety_delta_buffer = []


        #safety_mean = np.mean(self.ep_safety_buffer)
        for i in range(self.n_safety_epochs):
            approx_kl_divs = []
            approx_dl_divs = []

            #safety_delta_pessimistic, safety_delta = approx_safety_delta(self)
            safety_delta_buffer = [0]
            # if safety_mean + safety_delta/5 > 0.95:
            #     safety_delta_buffer.append(safety_delta)
            #     break
            last_safety_epoch_step_index = i
            # Do a complete pass on the rollout buffer
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                safety_loss = th.max(safety_loss1, safety_loss2).mean()
        ...
        self.logger.record("train/safety_epochs", last_safety_epoch_step_index)
...
class SaveHyperCallBack(BaseCallback):
    def __init__(self, ppo_safe: SafePPO2):
