...
def get_gradient_norm(parameters):
    ...
    for p in parameters:
        ...
        param_norm = gradient.norm(2)
        total_norm += param_norm.item() ** 2
    total_norm = total_norm ** 0.5
    return total_norm


class SafePPO4(OnPolicyAlgorithm):
    """
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(self.batch_size):
            with th.no_grad():
                log_prob = self.policy.actions_log_prob(rollout_data.observations, rollout_data.actions)
                r = 1 - rollout_data.old_log_prob/log_prob
                safety_delta = (r * rollout_data.safety_return).mean()
                s.append(safety_delta.item())

        self.logger.record("train/safety_delta_before", np.mean(s))

        # train for n_epochs epochs
        for epoch in range(self.n_epochs):
            # if safe_episode_mean < 0.9:
            #     break
            approx_kl_divs = []
            approx_dl_divs = []
            safety_delta_buffer = []
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                with th.no_grad():
                    action_net = self.policy.mlp_extractor.__dict__.get("action_net")
                    action_net_parameters = [] if action_net is None else action_net.parameters()
                    combined_parameters = list(self.policy.mlp_extractor.policy_net.parameters()) + action_net_parameters
                    gradient_norm_squared = get_gradient_norm(combined_parameters)
                    self.policy.mlp_extractor.policy_net.zero_grad()

                policy_loss = policy_loss / gradient_norm_squared
                safety_value_loss = F.mse_loss(rollout_data.safety_return, safety_values)
        ...
        self.logger.record("train/safety_delta_after", np.mean(safety_delta_buffer))
...
class SaveHyperCallBack(BaseCallback):
    def __init__(self, ppo_safe: SafePPO4):
